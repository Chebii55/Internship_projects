...
const DashboardPersonalInfo = () => {
    ...
    useEffect(() => {
        const checkSession = async () => {
            try {
                ...
                const employeeResponse = await fetch(`/employees/${sessionJson.id}`);
    ...
    // Ensure employeeData is not undefined before rendering
    if (!employeeData) return <p className="text-red-500">Employee data is unavailable</p>;
            ...
            <div className="font-std mb-10 w-full rounded-2xl bg-white p-10 font-normal leading-relaxed text-gray-900 shadow-xl">
                <div className="flex flex-col md:flex-row">
                    ...
                    <div className="md:w-2/3 md:pl-8">
                        ...
                        <p className="text-gray-600 font-bold mb-6">{employeeData.job_title}</p>

                        <h2 className="text-xl font-semibold text-indigo-800 mb-4">Organization Information</h2>
                        <p className="text-gray-700 mb-6">
                            {employeeData.department} 
                        </p>
                        <h2 className="text-xl font-semibold text-indigo-800 mb-4">Employment Status</h2>
                        <p className="text-gray-700 mb-6">
